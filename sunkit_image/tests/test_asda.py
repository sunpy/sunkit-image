...
Purpose: test processes for ASDA.
...
def test_artificial():
    ...
    print("Center Location ({:6.2f}, {:6.2f}),".format(100, 100),
          "Detection is ({:6.2f}, {:6.2f})".format(center_edge['center'][0][0],
                                                   center_edge['center'][0][1]))
    print('Rotating Speed {:6.2f}, Detection is {:6.2f}'.format(vmax, vr[0]))
    print('Expanding Speed {:6.2f}, Detection is {:6.2f}'.format(vmax *
                                                                 ratio, ve[0]))
    print(('Center Speed ({:6.2f}, {:6.2f}), ' + \
          'Detection is ({:6.2f}, {:6.2f})').format(0., 0., vc[0][0], vc[0][1]))
    print('Radius {:6.2f}, Detection is {:6.2f}'.format(rmax,
                                                 center_edge['radius'][0]))
...
def test_real_data():
    ...
    for i in np.arange(n):
        ...
        peak_diff.append((center_edge['peak'][i] - correct['peak'][idx]) /
                         correct['peak'][idx] * 100)
        radius_diff.append((center_edge['radius'][i] -
                            correct['radius'][idx]) /
                            correct['radius'][idx] * 100)
    ...
    print('The relative difference (%) between swirls detected by this ' +
          'program and the correct one is as following:')
    print('----------------------')
    print(('Difference in Peak Gamma1 Value: max {:6.2f}%, mean {:6.2f}%,' + \
          ' min {:6.2f}%').format(np.max(peak_diff), np.mean(peak_diff),
                                 np.min(peak_diff)))
    print(('Difference in radius: max {:6.2f}%, mean {:6.2f}%,' + \
          ' min {:6.2f}%').format(np.max(radius_diff), np.mean(radius_diff),
                                 np.min(radius_diff)))
    print(('Difference in rotating speed: max {:6.2f}%, mean {:6.2f}%,' + \
          ' min {:6.2f}%').format(np.max(vr_diff), np.mean(vr_diff),
                                 np.min(vr_diff)))
    print(('Difference in expanding/shriking speed: max {:6.2f}%, ' + \
          'mean {:6.2f}%, min {:6.2f}').format(np.max(ve_diff), np.mean(ve_diff),
                                               np.min(ve_diff)))
    print(('Difference in average intensity: max {:6.2f}%, mean {:6.2f}%,' + \
          ' min {:6.2f}%').format(np.max(ia_diff), np.mean(ia_diff),
                                 np.min(ia_diff)))
