...
def wow(*args, **kwargs):
    ...
    large scale structures are attenuated and the small scale structures are reinforced, with the objective criterion
    that the variance at all scales must be equal. The algorithm has the added advantage to allow attenuation of the
    noise at the same time, thus avoiding the usual explosion of noise usually inherent to enhancement methods.

    .. note::
        * The filter was developed with the intention to find an objective criterion for enhancement. By default, there
          are therefore no free parameters to adjust, except for the denoising coefficients. It is however possible to
          set the synthesis weights to values other than 1 to tune the output. It is also possible to merge the output
          with a gamma-scaled version of the original image. The weight of the gamma scaled image can be adjusted using
          the parameter 'h'.
          to the type of input image (e.g. wavelength or channel). For most purposes, the weights can be set
          equal for all scales.
        * We don't deal with NaN (Not a Number) in this implementation.
