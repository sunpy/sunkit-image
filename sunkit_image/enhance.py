...
def mgn(
    ...
    * Huw Morgan and Miloslav Druckmüller.
    ...
    for s, weight in zip(sigma, weights, strict=True):
        ...
        conv = ndimage.gaussian_filter(data, sigma=s, truncate=truncate, mode="nearest")
        ...
        conv = data - conv
        sigmaw = ndimage.gaussian_filter(conv**2, sigma=s, truncate=truncate, mode="nearest")
        with np.errstate(invalid="ignore"):
            sigmaw = np.sqrt(sigmaw)
