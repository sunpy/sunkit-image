...
def occult2(smap, nsm1, rmin, lmin, nstruc, ngap, qthresh1, qthresh2):
    """
    ...
    * Markus J. Aschwanden, Bart De Pontieu, Eugene A. Katrukha.
    ...
    # BANDPASS FILTER
    image2 = _bandpass_filter(image, nsm1, nsm2)
    nx, ny = image2.shape
...
def _bandpass_filter(data, nsm1=1, nsm2=3):
    """
    ...
    if nsm1 <= 2:
        return data - _smooth(data, nsm2, "replace")
    if nsm1 >= 3:
        return _smooth(data, nsm1, "replace") - _smooth(data, nsm2, "replace")
    return None


def _smooth(data, width, nanopt="replace"):
    """
    ...
    * Emmalg's answer on stackoverflow https://stackoverflow.com/a/35777966
    ...
    for col in range(startrc, stopc):
        ...
        for row in range(startrc, stopr):
            ...
            # Extract the window
            window = data[startwr:stopwr, startwc:stopwc]
            if nanopt == "replace":
